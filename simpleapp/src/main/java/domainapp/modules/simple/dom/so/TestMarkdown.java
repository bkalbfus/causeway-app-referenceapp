...
@Table(
    ...
    uniqueConstraints = {
        @UniqueConstraint(name = "TestMarkdown__name__UNQ", columnNames = {"name"})
    }
)
@NamedQueries({
        @NamedQuery(
                name = TestMarkdown.NAMED_QUERY__FIND_BY_NAME_LIKE,
                query = "SELECT so " +
                        "FROM TestMarkdown so " +
                        "WHERE so.name LIKE :name"
        )
})
@EntityListeners(CausewayEntityListener.class)
@Named(SimpleModule.NAMESPACE + ".TestMarkdown")
...
public class TestMarkdown implements Comparable<TestMarkdown>, CalendarEventable {

    static final String NAMED_QUERY__FIND_BY_NAME_LIKE = "TestMarkdown.findByNameLike";
    ...
    public static TestMarkdown withName(final String name) {
        val testMarkdown = new TestMarkdown();
        testMarkdown.setName(name);
        return testMarkdown;
    }
    ...
    public TestMarkdown updateName(
    ...
    public TestMarkdown updateAttachment(
    ...
    private final static Comparator<TestMarkdown> comparator =
            Comparator.comparing(TestMarkdown::getName);

    @Override
    public int compareTo(final TestMarkdown other) {
